...
[train_gen1, train_gen2, train_gen3, train_gen4] = get_pcam_generators(r'C:\Users\20192823\Documents\3 jaar\Kwartiel 3\BIA')  #Jim
...
for j in range(batch_size):
    ...
    for i in range(iter_size):
        img = next(train_gen1)[0][j]
        ax[i].imshow(img)
        img = next(train_gen2)[0][j]
        ax[i].imshow(img)
        img = next(train_gen3)[0][j]
        ax[i].imshow(img)
        img = next(train_gen4)[0][j]
        ax[i].imshow(img)