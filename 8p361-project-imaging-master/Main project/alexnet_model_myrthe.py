...
def model_builder(hp):
    ...
    AlexNet.compile(SGD(learning_rate=hp_learning_rate, momentum=0.95), loss = 'binary_crossentropy', metrics=['accuracy'])
...
tuner = kt.BayesianOptimization(model_builder,
                     ...
                     max_trials=5,
                     directory='my_dir2',
                     project_name='myrthe_tunes_hp')

stop_early = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=5)

tuner.search(train_gen, epochs=10, validation_data=val_gen, callbacks=[stop_early])
...
# history = model.fit(train_gen, steps_per_epoch=train_steps, validation_data = val_gen, validation_steps = val_steps, epochs=10)
